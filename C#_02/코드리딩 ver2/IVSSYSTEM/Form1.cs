...
namespace IVSSYSTEM
{
    public partial class Form1 : Form
    {
        ...
        public void Form1_Load(object sender, EventArgs e)
        {
            ...
            MainForm.II_ToolGroup = MainForm.CAMERA1_Job.VisionTool as CogToolGroup;
            ...
            // 주름관
            //setting
            Pattern_Result_Score_1_Min.Text = String.Format("{0:0.000}", PV.Pattern_Result_Score_11_Min);
            Pattern_Result_Score_1_Max.Text = String.Format("{0:0.000}", PV.Pattern_Result_Score_11_Max);
            GetBlobs_Area_1_Min.Text = String.Format("{0:0.000}", PV.GetBlobs_Area_11_Min);
            GetBlobs_Area_1_Max.Text = String.Format("{0:0.000}", PV.GetBlobs_Area_11_Max);
            CogHistogramTool_Result_Mean_1_Min.Text = String.Format("{0:0.000}", PV.CogHistogramTool_Result_Mean_11_Min);
            CogHistogramTool_Result_Mean_1_Max.Text = String.Format("{0:0.000}", PV.CogHistogramTool_Result_Mean_11_Max);

            //bypass
            Bypass_1.Checked = PV.bBypass_11;
            
            //result
            Pattern_Result_Score_1_Result.Text = PV.Pattern_Result_Score_11;
            GetBlobs_Area_1_Result.Text = PV.GetBlobs_Area_11;
            CogHistogramTool_Result_Mean_1_Result.Text = PV.CogHistogramTool_Result_Mean_11;

            //offset
            Pattern_Result_Score_1_Offset.Text = String.Format("{0:0.000}", PV.Pattern_Result_Score_11_Offset);
            GetBlobs_Area_1_Offset.Text = String.Format("{0:0.000}", PV.GetBlobs_Area_11_Offset);
            CogHistogramTool_Result_Mean_1_Offset.Text = String.Format("{0:0.000}", PV.CogHistogramTool_Result_Mean_11_Offset);



            // 민자관
            //setting
            Pattern_Result_Score_2_Min.Text = String.Format("{0:0.000}", PV.Pattern_Result_Score_11_Min);
            Pattern_Result_Score_2_Max.Text = String.Format("{0:0.000}", PV.Pattern_Result_Score_11_Max);
            GetBlobs_Area_2_Min.Text = String.Format("{0:0.000}", PV.GetBlobs_Area_11_Min);
            GetBlobs_Area_2_Max.Text = String.Format("{0:0.000}", PV.GetBlobs_Area_11_Max);
            CogHistogramTool_Result_Mean_2_Min.Text = String.Format("{0:0.000}", PV.CogHistogramTool_Result_Mean_11_Min);
            CogHistogramTool_Result_Mean_2_Max.Text = String.Format("{0:0.000}", PV.CogHistogramTool_Result_Mean_11_Max);

            //bypass
            Bypass_2.Checked = PV.bBypass_11;

            //result
            Pattern_Result_Score_2_Result.Text = PV.Pattern_Result_Score_11;
            GetBlobs_Area_2_Result.Text = PV.GetBlobs_Area_11;
            CogHistogramTool_Result_Mean_2_Result.Text = PV.CogHistogramTool_Result_Mean_11;

            //offset
            Pattern_Result_Score_2_Offset.Text = String.Format("{0:0.000}", PV.Pattern_Result_Score_11_Offset);
            GetBlobs_Area_2_Offset.Text = String.Format("{0:0.000}", PV.GetBlobs_Area_11_Offset);
            CogHistogramTool_Result_Mean_2_Offset.Text = String.Format("{0:0.000}", PV.CogHistogramTool_Result_Mean_11_Offset);

        }


        // SaveData Bypass / Setting / Offset
        ...
        void ReWriteJob()
        {
            ...
            CogSerializer.SaveObjectToFile(MainForm.CAMERA1_JobManager, PV.CAMERA1_PATH, typeof(BinaryFormatter), CogSerializationOptionsConstants.All);
        }
        ...
        void ReWriteSettings()
        {
            ...
            StreamWriter sw1;
            string FileName_Bypass = PV.SettingDataPath + @"\Bypass1.txt";
            StreamWriter sw2;
            string FileName_Setting = PV.SettingDataPath + @"\Setting1.txt";
            StreamWriter sw3;
            string FileName_Offset = PV.SettingDataPath + @"\Offset1.txt";
            ...
            sw.WriteLine(date + "_" + time + ", 카메라1, 검사기준설정 - 상하한 변경, " + Pattern_Result_Score_1_Min.Text + "," + Pattern_Result_Score_1_Max.Text + "," + GetBlobs_Area_1_Min.Text + "," + GetBlobs_Area_1_Max.Text + "," + CogHistogramTool_Result_Mean_1_Min.Text + "," + CogHistogramTool_Result_Mean_1_Max.Text
                );

            // 바이패스 변경
            sw.WriteLine("시간, 위치, 세팅변경, 주름관");

            sw.WriteLine(date + "_" + time + ", 카메라1, 검사기준설정 - Bypass 변경, " + Bypass_1.Checked);

            // 오프셋 변경
            sw.WriteLine("시간, 위치, 세팅변경, 주름관 형상, 주름관 블랍, 주름관 표면밝기");

            sw.WriteLine(date + "_" + time + ", 카메라1, 검사기준설정 - Offset 변경, " + Pattern_Result_Score_1_Offset.Text + "," + GetBlobs_Area_1_Offset.Text + "," + CogHistogramTool_Result_Mean_1_Offset.Text
             );

            // 검출 기준 변경
            sw.WriteLine("시간, 위치, 세팅변경, 검출 민감도, 검출 최소크기");

            sw.WriteLine(date + "_" + time + ", 카메라1, 검사기준설정 - 검출 기준 변경, " + num_threshold.Value + "," + num_pixel.Value
             );
