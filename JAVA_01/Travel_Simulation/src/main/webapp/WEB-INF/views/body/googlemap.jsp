...
<html>
  ...
  <body>
		...
		<p>경로 최적화란 : 경유지를 선택하신 순서가 아닌 최적화 된 순서로 변경합니다.</p>
			...
			<button type="button" class="btn btn-primary" id="saveRouteBtn">경로 저장하기</button>
			<br>
			<input type="text" id="inputRouteSeq" placeholder="경로를 불러올 SEQ입력">
			<button type="button" class="btn btn-primary" id="callRouteBtn">경로 불러오기</button>
			...
			<input type="text" id="callBMId" placeholder="북마크 경로를 불러올 ID">
			<button type="button" class="btn btn-primary" id="callBookmarkRouteBtn">북마크 경로 불러오기</button>
			<div id="bookmark"></div>
	...
	$("#callBookmarkRouteBtn").on("click", function(){
		...
		for(var i=0; i<data.list.length; i++){
			...
			html += "<a href=# onclick='showBMroute("+data.list[i].BM_RT_SEQ+",mid)' style='color : black; text-decoration:none;'>" 
					+ i + "번째 북마크 : " +  datart2.list[0].BM_NICK + "</a><br>";
	...
	function showBMroute(getbmrtseq, mid){
	...
	$("#callRouteBtn").on("click", function(){
		json={}
		json.rtseqperplan = $("#inputRouteSeq").val();
		var data = getAjax("/board/selectRoute.json", json);
		console.log(data);
		console.log("");
		console.log("----------------------------------------------------");
	...
	$("#saveRouteBtn").on("click", function(){		//국가
    	
		if(DepartPosition != null){
			console.log("");
			console.log("----------------------------------------------------");
			console.log("---------saveRoute Button Click Console.log---------");
			console.log("----------------------------------------------------");
	        console.log("출발위치 lat, lng");															
	        console.log(DepartPosition.lat() +","+DepartPosition.lng());
			...
			//console.log(data[0].list.NEXTVAL);
			console.log(data.list[0].NEXTVAL);
			
			json.rtseqperplan= data.list[0].NEXTVAL;
			
			json.rtdeplat = DepartPosition.lat();
			json.rtdeplng = DepartPosition.lng();
			json.rtarrlat = ArrivePosition.lat();
			json.rtarrlng = ArrivePosition.lng();
			...
			if(waypts.length>0){
				for(var j=0;j<waypts.length;j++){
					...
					console.log(waypts[j].location.lat() + ","+waypts[j].location.lng());
		      		...
		      		json.rtwaypolat = waypts[j].location.lat();
					json.rtwaypolng = waypts[j].location.lng();
					var data = getAjax("./insertRoute.json", json);	
		        }
			}else{
				json.rtdeplat = DepartPosition.lat();
				json.rtdeplng = DepartPosition.lng();
				...
				json.rtwaypolat = "";
				json.rtwaypolng = "";
				var data = getAjax("/board/insertRoute.json", json);	
			}
		    
			
			if(data.addRoute == true){
				alert("경로 저장이 성공적으로 완료되었습니다.");
				location.href = "./maptest.do";
			}else{
				alert("경로 저장에 실패하였습니다.");
				return;
			}
